...
def proj_profit_key_with_roles_and_levels_but_high_lvls_first_w_lvl_up(proj_dict, proj):
    return (sum([lvl + 1 for _, lvl in proj_dict[proj]["roles_order"]]) * proj_dict[proj]["score"]) / proj_dict[proj]["days"] / len(proj_dict[proj]["roles_order"])


def ordered_by_profit_ratio_with_roles_and_levels_but_high_lvls_first_w_lvl_up(input_path):
    ...
    sorted_proj.sort(
        key=lambda proj: proj_profit_key_with_roles_and_levels_but_high_lvls_first_w_lvl_up(proj_dict, proj))
    sorted_proj.reverse()
    ...
    for proj in sorted_proj:
        ...
        if len(remaining_roles.keys()) == 0:
            ...
            for role, lvl in assigned_roles.keys():
                for contrib in assigned_roles[(role, lvl)]:
                    if role in cont[contrib] and cont[contrib][role] <= lvl:
                        cont[contrib][role] += 1
            continue
...
for tf in test_files:
    a_dic, a_ord = ordered_by_profit_ratio_with_roles_and_levels_but_high_lvls_first_w_lvl_up(
        tf)
