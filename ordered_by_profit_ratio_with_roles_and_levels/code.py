...
def proj_profit_key_with_roles_and_levels(proj_dict, proj):
  return proj_dict[proj]["score"] /proj_dict[proj]["days"] / sum([lvl + 1 for _, lvl in proj_dict[proj]["roles_order"]])

def ordered_by_profit_ratio_with_roles_and_levels(input_path):
    ...
    sorted_proj.sort(key=lambda proj: proj_profit_key_with_roles_and_levels(proj_dict, proj))
    sorted_proj.reverse()
...
for tf in test_files:
    a_dic, a_ord = ordered_by_profit_ratio_with_roles_and_levels(tf)
