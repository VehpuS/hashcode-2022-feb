...
def proj_profit_key_reversed(proj_dict, proj):
  return -1 * proj_dict[proj]["score"] /proj_dict[proj]["days"]
...
def ordered_by_profit_ratio_reverse(input_path):
    ...
    sorted_proj = sorted(proj_dict.keys(), key=lambda proj: proj_profit_key_reversed(proj_dict, proj))
